...
def test_prediction_random_model():
    ...
    responses = [client.post("/predict/", json=request) for _ in range(10)]
    ...
    for i in range(1, len(responses)):
        ...
        assert not pred_1 == approx(
            pred_2
        ), f"Predictions should be different: {pred_1} == {pred_2}"


def test_predict_deterministic_model():
    """Linear regression/neural net model should return the same prediction for the same input."""
    request = {
    ...
    for i in range(1, len(responses)):
        pred_1 = responses[i].json()["predicted_avg_rating"]
        pred_2 = responses[i - 1].json()["predicted_avg_rating"]
        assert pred_1 == approx(
            pred_2
        ), f"Predictions should be the same: {pred_1} != {pred_2}"
